...
from src.ddi_classifier    import DDIClassifier
...
DEFAULT_MODEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'models', 'ddi_model.pt')
DEFAULT_DATA_DIR   = os.path.join(os.path.dirname(__file__), '..', 'data', 'processed')

class DrugInsight:
    """
    Main DrugInsight class. Loads the trained model and provides prediction.
    """

    _instance = None   # for singleton pattern when used in Streamlit

    def __init__(self, model_path: Optional[str] = None, data_dir: Optional[str] = None, device: Optional[str] = None):
        ...
        self.classifier = DDIClassifier(extra_features=6).to(DEVICE)  # Strict 6 features for pre-trained model
    ...
    def _compute_fusion(self, context, ml_prob):
        ...
        has_pathways  = len(context.get('shared_pathways', [])) > 0
        ...
        fused_prob = (w_rule * rule_score + w_ml * ml_score + w_twosides * twosides_score)
        ...
        return {
            ...
            'evidence': {
                'drugbank': {
                    ...
                    'shared_pathways': context.get('shared_pathways', []),
    ...
    def resolve_drug(self, name: str) -> dict:
        try:
            db_id, canonical_name = self.feature_extractor.resolve_drug(name)
            return {
                'drugbank_id': db_id,
                'name':        canonical_name,
                'enzymes':     self.feature_extractor.drug_enzymes.get(db_id, []),
                'targets':     self.feature_extractor.drug_targets.get(db_id, []),
                'pathways':    self.feature_extractor.drug_pathways.get(db_id, []),
                'has_smiles':  db_id in self.smiles_dict,
            }
        except ValueError as e:
            return {'error': str(e)}
